...
{
    public partial class ListDismissFull : Form
    {
        public ListDismissFull()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Library.TestingInput(maskedTextBox1, maskedTextBox2) == true)
            {
                ...
                string sql = string.Format(Queries.GetQuery("ListDismissFull.sql"), maskedTextBox1.Text, 
                ...
                {
                    Excel.Print("ListDismissFull.xlt", "A3", new DataTable[] { table }, new ExcelParameter[] { new ExcelParameter("A1", string.Format("Список уволенных с {0} по {1}", maskedTextBox1.Text, maskedTextBox2.Text)) });
                }
                else
                {
