...
{
    public partial class ListVeteranDismiss : Form
    {
        public ListVeteranDismiss()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Library.TestingInput(maskedTextBox1, maskedTextBox2) == true)
            {
                ...
                string sql = string.Format(Queries.GetQuery("ListVeteranDismiss.sql"), maskedTextBox1.Text, 
                    maskedTextBox2.Text, Connect.Schema);
                OracleDataAdapter adapter = new OracleDataAdapter(sql, Connect.CurConnect);
                DataTable table = new DataTable();
                adapter.Fill(table);
                if (table.Rows.Count != 0)
                { Excel.Print("ListRetirerDismiss.xlt", "A3", new DataTable[] { table }, 
                    new ExcelParameter[] { new ExcelParameter("A1", 
                        string.Format("Список работников, уволенных с {0} по {1}", 
