...
{
    public partial class ListDismissForDir : Form
    {
        public ListDismissForDir()
        {
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Library.TestingInput(maskedTextBox1, maskedTextBox2) == true)
            {
                ...
                string sql = string.Format(Queries.GetQuery("ListDismissForDir.sql"), maskedTextBox1.Text, 
                ...
                {
                    Excel.Print("ListDismissForDir.xlt", "A3", new DataTable[] { table }, new ExcelParameter[] { new ExcelParameter("A1", string.Format("Список уволенных с {0} по {1}", maskedTextBox1.Text, maskedTextBox2.Text)) });
                }
                else
                {
