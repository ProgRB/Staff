...
namespace Kadr
{
    public partial class PersonalCard : Form
    {
        ...
        string per_num;
        bool flagEmp, flagTransfer, flagAdd, fSpecial, fPlace_Service, fMob_Order, fMatter_No_Service, flagSave;
        ...
        public PersonalCard(string _per_num, int _transfer_id, EMP_seq _emp, bool _flagEmp, bool _flagTransfer,
            bool _flagAdd, int _sing_comb, ListEmp _listEmp)
        {
            InitializeComponent();
            per_num = _per_num;
            emp = _emp;
            ...
            {           
                ...
                cbPos_Name.AddBindingSource(transfer, POSITION_seq.ColumnsName.POS_ID, 
                    new LinkArgument(FormMain.position, POSITION_seq.ColumnsName.POS_NAME));
            }
            else
            {
                cbSubdiv_Name.AddBindingSource(transfer, SUBDIV_seq.ColumnsName.SUBDIV_ID, 
                ...
                cbPos_Name.AddBindingSource(transfer, POSITION_seq.ColumnsName.POS_ID, 
                    new LinkArgument(FormMain.allPosition, POSITION_seq.ColumnsName.POS_NAME));
            }
            ...
            if (!flagAdd)
            {
                ...
                {
                    tbCode_Subdiv.Text = FormMain.allSubdiv.Where(s => s.SUBDIV_ID.ToString() == cbSubdiv_Name.SelectedValue.ToString()).FirstOrDefault().CODE_SUBDIV.ToString();
                    tbCode_Pos.Text = FormMain.allPosition.Where(s => s.POS_ID.ToString() == cbPos_Name.SelectedValue.ToString()).FirstOrDefault().CODE_POS.ToString();
                }
            }
            ...
            if (!FormMain.flagArchive && flagAdd)
            {
        ...
        private void btExit_Click(object sender, EventArgs e)
        {
            if (btExit.Text == "Выход")
            {
                ...
                if (!GrantedRoles.GetGrantedRole("STAFF_VIEW"))
                {
                    ...
                    try
                    {
                        if (listEmp != null && !FormMain.flagArchive)
                        {
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            ...
            if (flagEdit && !fl_EditMol)
            {
                    ...
                    /*&& ((TRANSFER_obj)((CurrencyManager)BindingContext[transfer]).Current).HIRE_SIGN == true*/)
                {
                    ...
                    FormMain.employees.UpdateEmployee(new PercoXML.Employee(r_empNew.PERCO_SYNC_ID.ToString(), r_empNew.PER_NUM,
        ...
        {
            f_AddEdu = true;            
            formedu = new Edu(per_num, f_AddEdu, BindingContext[edu].Position, edu, speciality, instit, type_study, type_edu, qual, group_spec, dgEdu, pnVisible);
        ...
        {
            f_AddRelative = true;
            formfamily = new Family(per_num, f_AddRelative, BindingContext[relative].Position, relative, rel_type, dgFamily, pnVisible, emp);
        ...
        {
            f_AddKnow_lang = true;
            formknow_lang = new Know_Lang(per_num, f_AddKnow_lang, BindingContext[know_lang].Position, know_lang, lang, level_know, dgKnow_Lang, pnVisible);
        ...
        {
            f_AddReward = true;
            formrewards = new Rewards(per_num, f_AddReward, BindingContext[reward].Position, reward, reward_name,  base_doc, dgRewards, pnVisible);
        ...
        {
            f_AddEmp_priv = true;
            formemp_priv = new Emp_Priv(per_num, f_AddEmp_priv, BindingContext[emp_priv].Position, emp_priv, type_priv, base_doc, dgEmp_Priv, pnVisible);
        ...
        {
            f_AddAttest = true;
            formattest = new Attest(per_num, f_AddAttest, BindingContext[attest].Position, attest, base_doc, dgAttest, pnVisible);
        ...
        private void btAddProf_Train_Click(object sender, EventArgs e)
        {
            ...
            formprof_train = new Prof_Train(per_num, f_AddProf_train, BindingContext[prof_train].Position, prof_train, base_doc, dgProf_Train, pnVisible);
        ...
        private void btAddRise_Qual_Click(object sender, EventArgs e)
        {
            ...
            formrise_qual = new Rise_Qual(per_num, f_AddRise_qual, BindingContext[rise_qual].Position, 
                rise_qual, base_doc, instit, type_rise_qual, dgRise_Qual, pnVisible, true);
        ...
        private void btAddCompulsary_edu_Click(object sender, EventArgs e)
        {
            ...
            formrise_qual = new Rise_Qual(per_num, f_AddCompulsory_edu, BindingContext[compulsary_edu].Position,
                compulsary_edu, base_doc, instit, type_rise_qual, dgCompulsory_edu, pnVisible, false);
        ...
        private void btAddPostG_Study_Click(object sender, EventArgs e)
        {
            ...
            formpostg_study = new PostG_Study(per_num, f_AddPostg_study, BindingContext[postg_study].Position, postg_study, instit, type_postg_study, dgPostG_Study, pnVisible);
        ...
        private void btOrder_Click(object sender, EventArgs e)
        {
            if (passport.Count != 0)
            {
                ...
                else
                {
                    ...
                    if (!flagTransfer)
                    {
                        transferHire.Fill(string.Format(" where {0} = '{1}' and {2} = 0 and {3} = 1 and {4} = " +
                            "(select max({4}) from {5}.transfer where {0} = '{1}' and {2} = 0 and {3} = 1)",
