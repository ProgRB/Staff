...
select rownum,(*) from (select code_pos "Код профессии",pos_name "Профессия",
    to_char(date_begin_staff,'DD.MM.YYYY')||case when date_end_staff is null then null else '-'||to_char(date_end_staff,'DD.MM.YYYY') end ,count(s.staffs_id) "Количество единиц",     
     classific "Разряд", 
     case when nvl(t.salary,0)<=s2.tar_schema then null else s2.tar_schema end  "Тарифный коэфф. по схеме", 
     case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end "Тарифная ставка",
     case when s.classific is null then round(case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end*tarif.val) 
            else (case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end)*tarif.val end  "рубл.",
     ...
     (nvl(case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end,0)*(1+nvl(s.add_exp_area,0))+nvl(t.comb_addition,0))*tarif.val "Мес. фонд. ЗП",
        case 
            when code_tariff_grid||tg.tariff_grid_name is not null then  '('||code_tariff_grid||') '||tg.tariff_grid_name
            else null
        end     "Тарифная сетка",null
        ...
        join (select s3.staffs_id,case 
                                      when classific is null then s3.tar_by_schema
                                      else 
                                           (
                                           select tar_sal   from {0}.descr_tariff_grid dt  
                                           where dt.tariff_grid_id=s3.tarif_grid_id and dt.tar_classif=s3.classific and   
                ...
                from {0}.staffs s3)  s2 on (s.staffs_id=s2.staffs_id)
     ...
     group by tarif.val,code_pos,pos_name,s.classific, 
              case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end, 
              case when nvl(t.salary,0)<=s2.tar_schema then null else s2.tar_schema end, 
 ...
 UNION ALL
    ...
    round(AVG(classific),2) "Разряд", 
     null schema ,
     sum ( case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end), 
     SUM(case when s.classific is null then round(case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end*tarif.val) 
            else (case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end)*tarif.val end ) "рубл.",    
     ...
     sum((nvl(case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end,0)*(1+nvl(s.add_exp_area,0))+nvl(t.comb_addition,0))*tarif.val) "Мес. фонд. ЗП",
     null,null
  from 
            (select tariff val from {0}.base_tariff where bdate=(select max(bdate) from {0}.base_tariff where bdate<SYSDATE)) tarif,         
         (select staffs_id from {0}.staffs  
...
select * from (
    ...
    order by  mod(to_number(substr(code_pos,1,1))+4,3),nvl(case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end,0) desc 
    )
  UNION ALL
    ...
    count(s.staffs_id) "Количество единиц",     
     ...
     null schema ,
     sum(case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary  end) "stavka", 
     SUM(case when s.classific is null then round(case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end*tarif.val) 
            else (case when nvl(t.salary,0)<s2.tar_schema then s2.tar_schema else t.salary end)*tarif.val end ) "рубл.",    
     sum(t.comb_addition) "Надбавка за совм", 
     sum(t.comb_addition*tarif.val) "руб.",
