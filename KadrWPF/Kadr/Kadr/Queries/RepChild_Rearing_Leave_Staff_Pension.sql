...
select DISTINCT FIRST_VALUE(TP.CODE_SUBDIV) OVER(PARTITION BY WORKER_ID ORDER BY DATE_TRANSFER DESC) CODE_SUBDIV,
    ...
    DATE_BEGIN_LEAVE_1_5, DATE_END_LEAVE_1_5, DATE_BEGIN_LEAVE_3, DATE_END_LEAVE_3, NOTE
from 
(
    select PER_NUM, 1 RN, GREATEST(DATE_BEGIN_LEAVE,:p_beginPeriod) DATE_BEGIN_LEAVE_1_5, 
        LEAST(DECODE(NVL(CRL.BREAKPOINT_LEAVE,0),0,CRL.DATE_END_LEAVE,CRL.DATE_DEPARTURE-1),TRUNC(:p_endPeriod)) DATE_END_LEAVE_1_5, null DATE_BEGIN_LEAVE_3, 
        null DATE_END_LEAVE_3, 'ДЕТИ' NOTE
    ...
    select PER_NUM, 2 RN, null DATE_BEGIN_LEAVE_1_5, null DATE_END_LEAVE_1_5, 
        GREATEST(DATE_BEGIN_LEAVE, :p_beginPeriod) DATE_BEGIN_LEAVE_3, 
        LEAST(DECODE(NVL(CRL.BREAKPOINT_LEAVE,0),0,CRL.DATE_END_LEAVE,CRL.DATE_DEPARTURE-1), TRUNC(:p_endPeriod)) DATE_END_LEAVE_3, 'ДЛДЕТИ' NOTE
    from {0}.CHILD_CARE_LEAVE CRL
    JOIN {0}.RELATIVE using(RELATIVE_ID)
    where CRL.DATE_BEGIN_LEAVE <= :p_endPeriod and 
        DECODE(NVL(CRL.BREAKPOINT_LEAVE,0),0,CRL.DATE_END_LEAVE,CRL.DATE_DEPARTURE-1) >= :p_beginPeriod
