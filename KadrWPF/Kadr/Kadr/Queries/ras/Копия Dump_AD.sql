SELECT * FROM (
    SELECT T0.CODE_SUBDIV as SC, E.PER_NUM as TN, 
		...
		case when T0.TYPE_TRANSFER = 1 then {0}.GET_SIGN_PROFUNION(T.WORKER_ID, :p_date_dump) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) 
                then {0}.GET_SIGN_PROFUNION(T.WORKER_ID, :p_date_dump) else 0 end end as PRF,
        nvl(AD.SIGN_ADD,0) as PRN, 
        trim(to_char(nvl(AD.SALARY,0)*1000,'000000')) as OKL, 
        ...
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.TAX_CODE,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.TAX_CODE,0),'00')) else '00' end end as NAL, 
        /*trim(to_char(nvl(AD.PERCENT_ALIM,0),'000'))*/'000' as AL, 
        trim(to_char(nvl(AD.HARMFUL_ADDITION,0),'00000')) as VRED, 
        ...
        trim(to_char(nvl(AD.PERCENT13*10,0),'000')) as P13,
        /* Выбираем иждивенцев. Обнуляем данные тем кто уволен более месяца назад и тем кто переведен. */
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP14,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP14,0),'00')) else '00' end end as KI114,
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP15,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP15,0),'00')) else '00' end end as KI115,
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP16,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP16,0),'00')) else '00' end end as KI116,
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP17,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP17,0),'00')) else '00' end end as KI117,
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP18,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP18,0),'00')) else '00' end end as KI118,
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP19,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP19,0),'00')) else '00' end end as KI119,
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP20,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP20,0),'00')) else '00' end end as KI120,
        case when T0.TYPE_TRANSFER = 1 then trim(to_char(nvl(AD.COUNT_DEP21,0),'00')) else 
            case when T0.TYPE_TRANSFER = 3 and T0.DATE_TRANSFER >= ADD_MONTHS(:p_date_dump, -1) then trim(to_char(nvl(AD.COUNT_DEP21,0),'00')) else '00' end end as KI121, 
