...
select
    ...
    last_period,
	plan_begin,
	to_char(period_end,'DD.MM.YYYY') to_be_period,
  ...
  where t.subdiv_id in (select subdiv_id from {0}.subdiv  start with subdiv_id=:p_subdiv connect by prior subdiv_id=parent_id) 
 order by 4