...
namespace Tabel
{
    ...
    {        
        ...
        public Work_Pay_Type(string _per_num, 
            decimal _worked_day_id, decimal _transfer_id, DateTime _date, Table _table,
            DOC_LIST_seq _doc_list, decimal _degree_id, decimal worker_id)
        {
            InitializeComponent();            
            ...
            if (((Table)formTable).dgEmp.CurrentRow.Cells["FL_WAYBILL"].Value == DBNull.Value)
            {
            ...
            dtReg_doc.SelectCommand.Parameters.Add("p_transfer_id", transfer_id);
                ...
                for (int i = 0; i < dsChanges.Tables[0].Rows.Count; ++i)
                {
                        ...
                        dsChanges.Tables[0].Rows[i]["REG_DOC_ID"] == DBNull.Value)
                    {
                        ...
                        if (pay_type_id == 302)
                        {
                            ...
                            if (Table.HoursAbsence - hoursDoc + timeDoc < 0 &&//-10 &&
        ...
        private void tsbAddReg_Doc_Click(object sender, EventArgs e)
        {
            ...
            object waybill = formTable.dgEmp.CurrentRow.Cells["FL_WAYBILL"].Value;
