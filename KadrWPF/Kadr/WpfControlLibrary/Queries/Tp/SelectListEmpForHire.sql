            ...
            (SELECT T.*, (SELECT max(TRANSFER_ID) FROM APSTAFF.Transfer T2
                          WHERE T2.PER_NUM = T.PER_NUM and T2.SUBDIV_ID = T.SUBDIV_ID 
                                and ( 
                                    ...
                                    (T2.SIGN_CUR_WORK = 1 and T2.SIGN_COMB = 1 and 
										not Exists(SELECT * FROM APSTAFF.Transfer 
                                        WHERE PER_NUM = T2.PER_NUM and SUBDIV_ID = T2.SUBDIV_ID 
                ...
                (SELECT PER_NUM, SUBDIV_ID, POS_ID, DATE_HIRE, SIGN_COMB FROM APSTAFF.Transfer  
                 ...
                 GROUP BY PER_NUM, SUBDIV_ID, POS_ID, DATE_HIRE, SIGN_COMB
				) T
			) T1
