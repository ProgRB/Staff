...
namespace LibraryKadr
{
    public partial class ReportViewerWindow : Form
    {
        ...
        public static void ShowReport(string nameReport, string path, DataTable[] tables, IEnumerable<ReportParameter> r, System.Drawing.Printing.Duplex duplex = System.Drawing.Printing.Duplex.Simplex, string[] exportFormats = null)
        {
            ...
            f.Text += nameReport;
            f._reportViewer.LocalReport.LoadReportDefinition(File.Open(System.Windows.Forms.Application.StartupPath + @"\Reports\" + path, FileMode.Open, FileAccess.Read, FileShare.Read));
        ...
        /// <param name="tables">Таблица с данными</param>
        /// <param name="r">Параметры отчета</param>
        public static void RenderToExcel(IntPtr handle, string path, DataTable table, List<ReportParameter> r)
        {
            Form parentForm = new Form();
            var control = Form.FromHandle(handle); 
            //parentForm.Owner = 
            RenderToExcel(parentForm, path, new DataTable[] { table }, r);
        }
        ...
        /// <param name="tables">Таблица с данными</param>
        /// <param name="r">Параметры отчета</param>
        public static void RenderToExcel(Window window, string path, DataTable table, List<ReportParameter> r, string nameTempFile = "")
        {
            Form parentForm = new Form();
            WindowInteropHelper wih = new WindowInteropHelper(window);
            wih.Owner = parentForm.Handle;
            //parentForm.Owner = 
            RenderToExcel(parentForm, path, table, r, nameTempFile);
        }
        ...
        /// <param name="tables">Таблица с данными</param>
        /// <param name="r">Параметры отчета</param>
        public static void RenderToExcel(Window window, string path, DataTable[] tables, List<ReportParameter> r, string nameTempFile = "", string extensionFile = "xls")
        {
            Form parentForm = new Form();
            WindowInteropHelper wih = new WindowInteropHelper(window);
            wih.Owner = parentForm.Handle;
            //parentForm.Owner = 
            RenderToExcel(parentForm, path, tables, r, nameTempFile, extensionFile);
        }
        ...
        /// <param name="tables">Таблица с данными</param>
        /// <param name="r">Параметры отчета</param>
        public static void RenderToExcel(Form sender, string path, DataTable table, List<ReportParameter> r, bool withFormulas)
        {
            RenderToExcel(sender, path, new DataTable[] { table }, r, "", withFormulas, "xls");
        }
        ...
        {
            RenderToExcel(sender, path, new DataTable[] { table }, r, nameTempFile, false, extensionFile);
        }          
        ...
        /// <param name="r">Параметры отчета</param>
        /// <param name="nameTempFile">Имя для выгружаемого файла (без расширения)</param>
        public static void RenderToExcel(Form sender, string path, DataTable[] tables, List<ReportParameter> r, string nameTempFile, string extensionFile = "xls")
        {
            RenderToExcel(sender, path, tables, r, nameTempFile, false, extensionFile);
        }  
        ...
        public static void RenderToExcel(Form sender, string path, DataTable[] tables, List<ReportParameter> r,
            string nameTempFile = "", bool withFormulas = false, string extensionFile = "xls")
        {
            ...
            bw.DoWork += (bwk, e) =>
                {
                    ...
                    f.LoadReportDefinition(File.Open(System.Windows.Forms.Application.StartupPath + @"\Reports\" + path, FileMode.Open, FileAccess.Read, FileShare.Read));
                    f.DataSources.Clear();
                    //f.repViewer.LocalReport.
            ...
            bw.RunWorkerCompleted += (bwk, e) =>
                {
                    ...
                    else
                    {
                        ...
                        if (String.IsNullOrEmpty(nameTempFile))
                        {
                            bool fl = true;
        ...
        /// <summary>
        /// Выгрузка отчета в Эксель и использованием AbortableBackgroundWorker
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="path">Имя шаблона отчета</param>
        /// <param name="tables">Таблицы с данными</param>
        /// <param name="r">Параметры отчета</param>
        public static void RenderToExcelWPF(DependencyObject sender, string path, DataTable[] tables, IEnumerable<ReportParameter> r,
            string nameTempFile = "", bool withFormulas = false, string extensionFile = "xls")
        {
            Salary.Helpers.AbortableBackgroundWorker.RunAsyncWithWaitDialog(sender, "Формирование данных...",
                 (bwk, e) =>
            {
                LocalReport f = new LocalReport();
                f.LoadReportDefinition(File.Open(System.Windows.Forms.Application.StartupPath + @"\Reports\" + path, FileMode.Open, FileAccess.Read, FileShare.Read));
                ...
                else
                {
                    ...
                    if (String.IsNullOrEmpty(nameTempFile))
                    {
                        ...
                        while (fl)
                        {
                            ...
                            {
                                System.Windows.MessageBox.Show(Window.GetWindow(sender), ex.Message, "Ошибка сохранения");
                            }
                        }
                    }
                    else
                    ...
                    if (file_saved)
                        ...
                        {
                            System.Windows.MessageBox.Show(ex.Message, "Ошибка запуска файла");
                        }
