...
{
    public static class ClearContentControls
    {
        public static void ClearControls(System.Windows.Forms.Control control)
        {
            foreach (System.Windows.Forms.Control component in control.Controls)
            {
                if (component is GroupBox || component is Panel || component is TabControl || component is TabPage)
                    ClearControls(component);
                else
                {
                    switch (component.GetType().Name)
                    {
                        ...
                        case "ComboBox":
                            ...
                            break;
                        case "DateEditor":
                            ((DateEditor)component).Date = null;
                            break;
                        case "NumericUpDown":
                            ((NumericUpDown)component).Value = 1;
                            break;
                        case "CheckBox":
                            if (((CheckBox)component).ThreeState == true)
                                ((CheckBox)component).CheckState = CheckState.Indeterminate;
                            else
                    ...
                    //if (component is System.Windows.Forms.TextBox || component is System.Windows.Forms.MaskedTextBox)
                    //{
                    //    component.Text = "";
                    //}
                    //else
                    //    if (component is System.Windows.Forms.ComboBox)
                    //    {
                    //        ((ComboBox)component).SelectedItem = null;
                    //    }
                }
    ...
    public class Library
    {
            ...
            int _gr_work_id, DateTime _date_transfer, int _num_day)
        {
            ...
            com.CommandText = string.Format(Queries.GetQuery("Table/UpdateGr_Work.sql"), Connect.SchemaApstaff);
            com.BindByName = true;
            ...
            if (control != null)
            {
                if (control.Controls.Count != 0)
                {
                    foreach (Control component in control.Controls)
                    {
                        ...
                        {
                            ((DateTimePicker)component).Enabled = flag;
                            ((DateTimePicker)component).BackColor = color;
                        }
...
namespace LibraryKadr
{
    ...
    public class LinkKadr
    {
        ...
        public static bool CanExecuteByAccessSubdiv(object worker_id, string app_name)
        {
            try
            {
                OracleCommand a = new OracleCommand(string.Format(Queries.GetQuery("LinkCanExecute.sql"), Connect.SchemaApstaff), Connect.CurConnect);
                a.BindByName = true;
                a.Parameters.Add("p_worker_id", OracleDbType.Decimal, worker_id, ParameterDirection.Input);
    ...
    {
        public LinkData(string _per_num, decimal? _transfer_id, decimal? _worker_id)
        {
            Per_num = _per_num;
