...
namespace Kadr
{
    public partial class HBPosition : System.Windows.Forms.UserControl
    {
        ...
        private void btDelete_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Вы действительно хотите удалить запись?", "АСУ \"Кадры\"", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                if (DictionaryPerco.positions.DeletePosition(pos))
                {
                    position.Save();
                    Connect.Commit();
                }
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            if (tbCode_Pos.Text != "")
            {
                if (tbName_Pos.Text != "")
                {
                    ...
                    if (flagAdd)
                    {
                        if (DictionaryPerco.positions.InsertPosition(new Position(
                    ...
                    {
                        if (DictionaryPerco.positions.UpdatePosition(new Position(
                            ((POSITION_obj)((CurrencyManager)BindingContext[position]).Current).POS_ID.ToString(),
                            ((POSITION_obj)((CurrencyManager)BindingContext[position]).Current).POS_NAME)))
                        {
                            position.Save();
