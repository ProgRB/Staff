...
{
    public partial class ListRetirerStructSubdiv : Form
    {
        public ListRetirerStructSubdiv()
        {
            InitializeComponent();
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string sql = string.Format(Queries.GetQuery("RetirersBoss.sql"), maskedTextBox1.Text, Connect.Schema);
            OracleDataAdapter adapter = new OracleDataAdapter(sql, Connect.CurConnect);
            DataTable table = new DataTable();            
            adapter.Fill(table);
            adapter.SelectCommand.CommandText = string.Format(Queries.GetQuery("RetirersSpec.sql"), maskedTextBox1.Text, 
                Connect.Schema);
            adapter.Fill(table);
            adapter.SelectCommand.CommandText = string.Format(Queries.GetQuery("RetirersOther.sql"), maskedTextBox1.Text, 
                Connect.Schema);
            adapter.Fill(table);
            adapter.SelectCommand.CommandText = string.Format(Queries.GetQuery("RetirersJob.sql"), maskedTextBox1.Text, 
                Connect.Schema);
            adapter.Fill(table);
            adapter.SelectCommand.CommandText = string.Format(Queries.GetQuery("RetirersTotalSubdiv.sql"), 
