...
namespace VacationSchedule
{
    public partial class ViewCard : Form
    {
        ...
        public static void NoteAccountReport(decimal? subdiv_current, decimal[] vac_sched_ids)
        {
            try
            {
                DataSet ds = new DataSet();
                using (OracleCommand cmd = new OracleCommand(string.Format("begin {0}.VAC_SCHED_PACK.GET_NOTE_ACCOUNT(:p_vac_consist_id,:c); END;", Connect.Schema), Connect.CurConnect))
                {
                    cmd.BindByName = true;
                ...
                if (Signes.Show(subdiv_current, "noteAccountVS", "Введите должность и ФИО работника кадровой службы(место подписи)", 1, ref s_pos) == true)
                {
        ...
        private void NoteAccount_Click(object sender, EventArgs e)
        {
            ...
            {
                NoteAccountReport(this.SubdivID,
        ...
        private void btReCalcPeriodsVS_Click(object sender, EventArgs e)
        {
            if (tabsTypeVS.SelectedTab != null)
            {
                ...
                if (d.Rows.Count > 0)
                {
                    RecalcPeriods f = new RecalcPeriods(decimal.Parse(transfer_id), (d.Rows[0].DataBoundItem as DataRowView).Row.Field<decimal>("VAC_GROUP_TYPE_ID"));
                   // f.Owner = WindowInterop
        ...
        public static void PrintOrderPlantReport(decimal? subdiv_id, string vac_ids)
        {
            string[][] s_pos = new string[][] { };
            if (Signes.Show(subdiv_id, "OrderPlantVacShed", "Ввод должность и ФИО на месте подписи", 1, ref s_pos) == true)
            {
        ...
        private void btPrintOrderPlantVS_Click(object sender, EventArgs e)
        {
            if (treeView.SelectedNode != null && treeView.SelectedNode.Level > 0)
            {
                PrintOrderPlantReport(this.SubdivID, (treeView.SelectedNode.Tag as DataRow)["vac_consist_id"].ToString());
            }
            else
                {
                    ToolTip t = new ToolTip();
        ...
        {
            if (SelectedVac != null)
                RepVacNotification( SubdivID, new Decimal[] { SelectedVac.Field2<Decimal>("VAC_SCHED_ID") });
            else
        ...
        {
            return LinkKadr.CanExecuteByAccessSubdiv(e.Worker_ID, "VS_VIEW");
        }
        ...
        public static void RepVacNotification(decimal subdivid, decimal[] vac_ids)
        {
            string[][] str = null;
            if (Signes.Show(subdivid, "VacNotification", "Выберите ответственные лица", 1, ref str) == true)
            {
                try
                {
