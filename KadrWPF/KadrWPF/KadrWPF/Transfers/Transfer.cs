...
namespace Kadr
{
    public partial class Transfer : Form
    {
            ...
            ListEmp _listEmp)
        {
            ...
            cbSubdiv_Name.AddBindingSource(transfer, SUBDIV_seq.ColumnsName.SUBDIV_ID, new LinkArgument(subdivLocal, SUBDIV_seq.ColumnsName.SUBDIV_NAME));
            tbCode_Subdiv.Text = subdivLocal.Where(s => s.SUBDIV_ID.ToString() == cbSubdiv_Name.SelectedValue.ToString()).FirstOrDefault().CODE_SUBDIV.ToString();
            cbPos_Name.AddBindingSource(transfer, POSITION_seq.ColumnsName.POS_ID, new LinkArgument(positionLocal, POSITION_seq.ColumnsName.POS_NAME));
            ...
            code_subdiv = subdivLocal.Where(s => s.SUBDIV_ID ==
        ...
        private void btSave_Click(object sender, EventArgs e)
        {
            ...
            if (!flagDismiss)
            {
                ...
                DictionaryPerco.employees.UpdateEmployee(new PercoXML.Employee(r_empNew.PERCO_SYNC_ID.ToString(),
                    r_empNew.PER_NUM,
            ...
            else
            {
                ...
                if (r_transferNew.SIGN_COMB == true)
                {
                    ...
                    {
                        DictionaryPerco.employees.DismissEmployee(r_empNew.PERCO_SYNC_ID.ToString(), r_empNew.PER_NUM,
        ...
        private void btPreview_Click(object sender, EventArgs e)
        {
            if (!flagDismiss)
            {
                ...
                if (Signes.Show(0, "Transfer_Order", "Выберите должностное лицо для подписи приказа", 1, ref s_pos) == true)
                {
        ...
        private void btViewAdd_Agreement_Click(object sender, EventArgs e)
        {
            ...
            if (Signes.Show(0, "Transfer_Order", "Выберите должностное лицо для подписи", 1, ref _dt) == true)
            {
                DataSet _dsReport = new DataSet();
                _dsReport.Tables.Add("SIGNES");
                _dsReport.Tables.Add("ORDER_EMP");
