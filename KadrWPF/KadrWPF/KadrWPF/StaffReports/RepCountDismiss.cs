...
{
    public partial class RepCountDismiss : Form
    {
        public RepCountDismiss()
        {
            InitializeComponent();                 
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Library.TestingInput(maskedTextBox1, maskedTextBox2) == true)
            {
                ...
                string sql = string.Format(Queries.GetQuery("RepCountDismiss.sql"), maskedTextBox1.Text, 
                ...
                {
                    Excel.Print("RepCountDismiss.xlt", "A4", 
                        new DataTable[] { table }, 
                        new ExcelParameter[] { new ExcelParameter("A1", string.Format("Отчёт о количестве уволенных с {0} по {1}", maskedTextBox1.Text, maskedTextBox2.Text)) },
                        false);
                }
                else
                {
