...
from (
    select DECODE(GROUPING(CODE_SUBDIV),0,LPAD(CODE_SUBDIV,7,' '),'  Итого') CODE_SUBDIV, 
        ROUND(SUM(KT01),0) KT01, ROUND(SUM(KT02),0) KT02, ROUND(SUM(KT04),0) KT04, ROUND(SUM(KT08),0) KT08,
        ROUND(SUM(KT09),0) KT09, ROUND(SUM(KTPr),0) KTPr, ROUND(SUM(KTRuk),0) KTRuk
    from 
    (
        select CODE_SUBDIV, 
            DECODE(DEGREE_ID,1,COEFF,0) as kt01,
            DECODE(DEGREE_ID,2,COEFF,0) as kt02,
            CASE WHEN DEGREE_ID = 4 and SIGN_POS != '2' THEN COEFF ELSE 0 END as kt04,
            DECODE(DEGREE_ID,8,COEFF,0) as kt08,
            DECODE(DEGREE_ID,9,COEFF,0) as kt09,
            CASE WHEN DEGREE_ID not in (1,2,4,8,9) THEN COEFF ELSE 0 END as KTPr,
            CASE WHEN DEGREE_ID = 4 and SIGN_POS = '2' THEN COEFF ELSE 0 END as KTRuk
        from
        (    
            with SUBD as 
                (select * from 
            ...
            select distinct FIRST_VALUE(CODE_SUBDIV) OVER(PARTITION BY PER_NUM,CODE_SUBDIV ORDER BY DATE_TRANSFER DESC) CODE_SUBDIV, 
				...
				(END_TRANSFER+1/86400-DATE_TRANSFER)/(:p_date_end+1/86400-:p_date_begin) COEFF
            from 
            (
                ...
                from (select TRANSFER_ID, PER_NUM, SUBDIV_ID, POS_ID, DEGREE_ID, 
						GREATEST(TRUNC(DATE_TRANSFER),:p_date_begin) DATE_TRANSFER, 
                        LEAST(NVL((select DECODE(T1.TYPE_TRANSFER_ID,3,TRUNC(DATE_TRANSFER)+86399/86400,TRUNC(DATE_TRANSFER)-1/86400) 
                        from {0}.TRANSFER T1 where T1.FROM_POSITION = T.TRANSFER_ID),DATE '3000-01-01'),:p_date_end) END_TRANSFER
