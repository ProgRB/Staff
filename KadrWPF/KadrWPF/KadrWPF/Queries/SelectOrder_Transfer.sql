with APPR as (
        ...
        join {0}.PROJECT_TRANSFER PT using(PROJECT_TRANSFER_ID)
        join {0}.PROJECT_PLAN_APPROVAL PPA on (PA.PROJECT_PLAN_APPROVAL_ID=PPA.PROJECT_PLAN_APPROVAL_ID)
    ...
    WHERE PT.TRANSFER_ID = :p_TRANSFER_ID and TYPE_TERM = 2
    )
select E.PER_NUM, E.EMP_LAST_NAME, E.EMP_FIRST_NAME, E.EMP_MIDDLE_NAME,
    ...
    NVL(PT.DATE_END_CONTR, (SELECT DATE_TERM_TRANSFER FROM TERM)) DATE_TERM_TRANSFER,
    ...
    A.CLASSIFIC, A.SALARY,
    (select TG.CODE_TARIFF_GRID from {0}.TARIFF_GRID TG 
    where TG.TARIFF_GRID_ID = A.TARIFF_GRID_ID) CODE_TARIFF_GRID,
    Rtrim(CASE WHEN A.COMB_ADDITION > 0 THEN ' совмещение - '||A.COMB_ADDITION||';' ELSE '' END ||
        CASE WHEN A.HARMFUL_ADDITION_ADD > 0 THEN ' вредность доп. - '||A.HARMFUL_ADDITION_ADD||';' ELSE '' END, ';')
    NOTE_ADDITION_BY_HIRE, A.COMB_ADDITION, A.HARMFUL_ADDITION_ADD,
    ...
    A.SIGN_ADD, A.DATE_ADD, PT.PROBA_PERIOD, PT.CONTR_EMP, PT.DATE_CONTR,    
...
from {0}.TRANSFER PT
join {0}.ACCOUNT_DATA A ON PT.TRANSFER_ID = A.TRANSFER_ID
