...
where sub.code_subdiv=lpad('{0}',3,'0') and tr.subdiv_id=sub.subdiv_id and r.per_num=TR.PER_NUM and 
...
where sub.code_subdiv=lpad('{0}',3,'0') and tr.subdiv_id=sub.subdiv_id and r.per_num=TR.PER_NUM and emp.per_num=tr.per_num and 
upper(emp.emp_sex)='ф' and R.REWARD_NAME_ID in (select RN.REWARD_NAME_ID from {1}.REWARD_NAME RN
        where upper(RN.REWARD_NAME) like '%берепюм%рпсдю%') and 
