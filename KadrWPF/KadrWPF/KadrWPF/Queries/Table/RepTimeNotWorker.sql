select GROUPING(NUM_MONTH) GR1, 
    DECODE(GROUPING(NUM_MONTH),0,TO_CHAR(NUM_MONTH),1,'ИТОГО') NUM_MONTH, 
...
from (
    select EXTRACT(MONTH FROM ST.START_PERIOD) NUM_MONTH, EXTRACT(DAY FROM LAST_DAY(ST.START_PERIOD)) DAY_IN_MONTH,
    ...
    WHERE ST.APP_NAME_ID = 1 and ST.START_PERIOD<=:p_end_date and ST.END_PERIOD>=TRUNC(:p_begin_date, 'YEAR') --and T.SUBDIV_ID = 44
        and ST.SIGN_APPENDIX = 1 and ST.SIGN_COMB = 0 
        and DA.DEG_PRIORITY in (SELECT COLUMN_VALUE FROM TABLE(:p_TABLE_DEG_PRIORITY))
    GROUP BY EXTRACT(MONTH FROM ST.START_PERIOD), EXTRACT(DAY FROM LAST_DAY(ST.START_PERIOD))
)
GROUP BY ROLLUP(NUM_MONTH)