select * from (select 
    case when LV.PER_NUM is not null then 
        ...
        then (select S.SUBDIV_NAME from {0}.fr_emp fr1 join {0}.subdiv s 
    ...
    case when LV.PER_NUM is not null then 
        ...
        then (select P.POS_NAME from {0}.fr_emp fr1 join {0}.position p 
                ...
                where FR1.PERCO_SYNC_ID = LV.PERCO_SYNC_ID) else 
        '' end
        end as pos_name,(select transfer_id from {0}.TRANSFER tr2 where EM.PER_NUM = TR2.PER_NUM and rownum = 1 and TR2.DATE_TRANSFER = 
     (select max(date_transfer) from {0}.TRANSFER tr3 where em.PER_NUM = TR3.PER_NUM)) as transfer_id, LV.VIOLATION_LOG_ID, LV.LIST_VIOLATOR_ID, LV.PERCO_SYNC_ID, LV.OTHER_VIOLATOR_ID    
