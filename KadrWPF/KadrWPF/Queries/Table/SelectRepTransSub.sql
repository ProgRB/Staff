...
from {0}.TRANSFER T0
join (select distinct AD.TRANSFER_ID,
            FIRST_VALUE(AD.SALARY) over (partition by AD.TRANSFER_ID order by AD.CHANGE_DATE DESC) SALARY
        from {0}.ACCOUNT_DATA AD) A0 
    on T0.TRANSFER_ID = A0.TRANSFER_ID 
left join {0}.TRANSFER T_OLD on T0.FROM_POSITION = T_OLD.TRANSFER_ID 
left join (select distinct AD.TRANSFER_ID,
            FIRST_VALUE(AD.SALARY) over (partition by AD.TRANSFER_ID order by AD.CHANGE_DATE DESC) SALARY
        from {0}.ACCOUNT_DATA AD) A_OLD 
    on T_OLD.TRANSFER_ID = A_OLD.TRANSFER_ID 
...
left join (select distinct AD.TRANSFER_ID,
            FIRST_VALUE(AD.SALARY) over (partition by AD.TRANSFER_ID order by AD.CHANGE_DATE DESC) SALARY
        from {0}.ACCOUNT_DATA AD) A_NEW 
    on T_NEW.TRANSFER_ID = A_NEW.TRANSFER_ID 
where T_NEW.DATE_TRANSFER between :p_date_begin and :p_date_end 
    and T0.SUBDIV_ID = :p_subdiv_id and T0.SUBDIV_ID != T_NEW.SUBDIV_ID
) V
