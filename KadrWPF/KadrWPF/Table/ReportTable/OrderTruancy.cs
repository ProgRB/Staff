...
namespace Tabel
{
    public partial class OrderTruancy : Form
    {
        public OrderTruancy()
        {
            InitializeComponent();
        ...
        private void btOrderTruancy_Click(object sender, EventArgs e)
        {
            ...
            odaData.SelectCommand.CommandText = string.Format(Queries.GetQuery("Table/OrderOnTruancyFio.sql"),
            ...
            try
            {
                ...
                m_ExcelApp.Workbooks.Open(Application.StartupPath + @"\Reports\OrderTruancy.xlt", oMissing, oMissing,
                oMissing, oMissing, oMissing, oMissing, oMissing, oMissing,
                oMissing, oMissing, oMissing, oMissing, oMissing, oMissing);
                m_Sheet = (WExcel._Worksheet)m_ExcelApp.Sheets[1];

                //Заполняем отдельные параметры
                m_Sheet.get_Range("A3", Type.Missing).Value2 = string.Format(
                ...
                odaData = new OracleDataAdapter("", Connect.CurConnect);
                odaData.SelectCommand.BindByName = true;
                odaData.SelectCommand.CommandText = string.Format(Queries.GetQuery("Table/OrderOnTruancyPlant.sql"),
                    DataSourceScheme.SchemeName);
