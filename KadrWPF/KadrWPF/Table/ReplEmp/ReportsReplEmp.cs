...
namespace Kadr.Shtat
{
    public partial class ReportsReplEmp : Form, INotifyPropertyChanged
    {
        ...
        {
            OracleDataAdapter adapt = new OracleDataAdapter(string.Format(Queries.GetQuery(@"Table\GetReplDataByPeriod.sql"),
                ...
                case TypeReportRepl.ServiseNote:
                    {
                        ...
                        if (Signes.Show(sub_sel_repl.subdiv_id, "ServiseNoteReplEmp", "Ввод должности и ФИО для подписи", 2, ref sp) == true)
                        {
                            ...
                            OracleDataAdapter adapt = new OracleDataAdapter(string.Format(Queries.GetQuery(@"Table\R_btServiseNoteReplShtat.sql"), DataSourceScheme.SchemeName, s), Connect.CurConnect);
                            DataTable table = new DataTable();
                            adapt.Fill(table);
                ...
                case TypeReportRepl.OrderPlant:
                    {
                        ...
                        if (Signes.Show(sub_sel_repl.subdiv_id, "ReplOrderPlantShtat", "Ввод должности и ФИО на месте подписи", 1, ref s_pos) == true)
                        {
                            OracleDataAdapter a = new OracleDataAdapter(string.Format(Queries.GetQuery("Table/R_btReplOrderPlant.SQL"), Connect.Schema), Connect.CurConnect);
                ...
                case TypeReportRepl.OrderSubdiv:
                    {
                        ...
                        if (Signes.Show(sub_sel_repl.subdiv_id,"ReplOrderSubdivShtat","Введите ФИО и должности ответственных лиц",1,ref pos) == true)
                        {
                            DataTable tsignes = new DataTable();
                            tsignes.Columns.Add("POS_NAME");
                            tsignes.Columns.Add("FIO");
                            ...
                            OracleDataAdapter a = new OracleDataAdapter(string.Format(Queries.GetQuery(@"Table\R_ReplOrderSubdiv.sql"), Connect.Schema, st), Connect.CurConnect);
                            a.SelectCommand.Parameters.Add("p_repl_id", OracleDbType.Array, l.Select(r => Convert.ToDecimal(r)).ToArray(), ParameterDirection.Input).UdtTypeName = "APSTAFF.TYPE_TABLE_NUMBER";
